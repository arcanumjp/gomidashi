...
import jp.arcanum.gomidashi.tomiya.activity.SettingsActivity;
...
public class MonthlyCalendarView extends View {
     ...
     * @param activity
     */
    public MonthlyCalendarView(Activity activity) {
    ...
    private void initResource(AttributeSet attrs) {
        ...
        this.holidayText.setColor(this.c_foregroud);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 地区名取得
        SharedPreferences sp = _parent.getSharedPreferences(SettingsActivity.class.getName(), Activity.MODE_PRIVATE);
        ...
        for (int row = 1; row < 7; row++) {
            for (int col = 0; col < 7; col++) {
                if (this.matrix[row-1][col] != null) {
                    ...
                    cal.set(
                            matrix[row-1][col].year,
                            matrix[row-1][col].month -1,
                            matrix[row-1][col].day
                    );

                    GomiInfo gomiInfo = LocalUtil.getGomiInfo(cal, localName);
                    if(gomiInfo.getTodayGomiName() != null){
                        canvas.drawText(
                                gomiInfo.getTodayGomiName()
